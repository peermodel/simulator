...
// SpaceRead AUTOMATON:
// --------------------------------------
func NewAutomaton_SpaceRead(automatonName string, createAutomatonFlag bool, a *Automaton) (*Automaton, *Machine) {
    // --------------------------------------
    ...
    // --------------------------------------
    // init: INIT STATE
    //   - GVars: [LinkNo, Wfid, Wid, Query, Vars, Wtxid, Pid, Cid]
    // --------------------------------------
    a.AddState("init", "SpaceRead(ctx.Cid, ctx.LinkNo, ctx.Pid, ctx.Query, ctx.Vars, ctx.Wfid, ctx.Wid, ctx.Wtxid): SpaceRead automaton", func(s *Status, m *Machine) StateRetEnum {
    ...
    // --------------------------------------
    // 1: CALL STATE
    //   - GVars: [Wid, Vars, RetErr, Pid, RetEs, Cid]
    // --------------------------------------
    a.AddState("1", "non guard link: read without tx", func(s *Status, m *Machine) StateRetEnum {
        ctx := m.Context.(*Context)
        ...
        foundAutomaton, foundFlag := s.CheckAutomatonExistence("Read") 
        theNewAutomaton, m1 := NewAutomaton_Read("Read", ! foundFlag, foundAutomaton) 
        if !foundFlag { 
    ...
    a.AddState("2", "guard link", func(s *Status, m *Machine) StateRetEnum {
        ...
        if (lvs.wtx.Txcc == PCC) { m.CurrentState = "4" } else { m.CurrentState = "6" }
    ...
    // --------------------------------------
    a.AddState("3", "exit", func(s *Status, m *Machine) StateRetEnum {
        ctx := m.Context.(*Context)
    ...
    // --------------------------------------
    a.AddState("4", "call pcc read", func(s *Status, m *Machine) StateRetEnum {
        ctx := m.Context.(*Context)
        ...
        // create and call new machine: 
        foundAutomaton, foundFlag := s.CheckAutomatonExistence("PccRead") 
        theNewAutomaton, m1 := NewAutomaton_PccRead("PccRead", ! foundFlag, foundAutomaton) 
        if !foundFlag { 
            s.AddAutomaton(theNewAutomaton)
        } 
        ctx2 := m.Context.Copy().(IContext) 
    ...
    //   - Aliases:[l]
    // --------------------------------------
    a.AddState("5", "if link type is guard: call transactional read; otherwise non-transactional read (for action, service, or service-in, service-out)", func(s *Status, m *Machine) StateRetEnum {
        lvs := m.LocalVariables.(*localVariables)
        
        // debug: 
        /**/ m.PrintlnX(TRACE0, TAB, "- l", lvs.l)
        
        if (lvs.l.Type == GUARD) { m.CurrentState = "2" } else { m.CurrentState = "1" }
    ...
    //   - GVars: [RetErr]
    // --------------------------------------
    a.AddState("6", "set error", func(s *Status, m *Machine) StateRetEnum {
        ctx := m.Context.(*Context)
        
        // debug: 
        /**/ m.PrintlnY(TRACE0, TAB, "- RetErr", ctx.RetErr)
        
        ctx.RetErr = errors.New("ill. txcc")
    ...
    //   - Aliases:[wtx, l]
    // --------------------------------------
    a.AddState("7", "init alias vars", func(s *Status, m *Machine) StateRetEnum {
        lvs := m.LocalVariables.(*localVariables)
        ctx := m.Context.(*Context)
